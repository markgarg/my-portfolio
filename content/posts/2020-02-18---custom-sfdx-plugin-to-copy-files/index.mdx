...
![Project directory when there's at rest](./proj-dir-empty.png)
_Project directory when there’s no deployment — notice `src/namedCredentials` is empty_
...
![Project directory when custom ant script runs for staging environment](./proj-dir-with-ant.png)
_Project directory when custom `ant` script runs for `staging` environment_
